...
#borrow and return
readRDS("Output/borrow_return_hourly.rds")
library(ggplot2)

f10 <- ggplot(
  borrow_return_hourly,
  aes(
    x = hour,
    y = count,
    color = type
  ...
  labs(
    title = "Borrow vs Return Demand by Hour",
    x = "Hour",
    y = "Number of Trips",
    color = ""
  ) +
...
# 騎乘距離比例
distance_data <- mrt_sub_Youbike_all %>%
  filter(!is.na(ride_distance_km))
f9 <- ggplot(
  distance_data,
  aes(x = ride_distance_km)
) +
  geom_density(
    fill = "#4E79A7",
    alpha = .5
  ...
  labs(
    title = "Distribution of MRT-related YouBike Riding Distance",
    subtitle = "Median = 0.73 km, 90th percentile = 1.87 km",
    x = "Riding Distance (km)",
    y = "Density"
  ) +
