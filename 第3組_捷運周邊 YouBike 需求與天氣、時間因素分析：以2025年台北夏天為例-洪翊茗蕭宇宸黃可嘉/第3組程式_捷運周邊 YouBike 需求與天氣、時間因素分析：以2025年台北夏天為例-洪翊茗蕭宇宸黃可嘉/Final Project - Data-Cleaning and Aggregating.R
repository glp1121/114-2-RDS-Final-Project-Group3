...
# =========================
# Borrow / Return 分開 + 騎乘距離
# =========================

# 建立 YouBike 站點座標表
clean_station_name <- function(x) {
  x %>%
    str_replace("^YouBike2.0_", "") %>%
    str_replace("_\\d+$", "") %>%
    str_trim()
}

bike_coord <- bike_station %>%
  mutate(
    sna_clean = clean_station_name(sna)
  ) %>%
  group_by(sna_clean) %>%
  summarise(
...
mrt_sub_Youbike_all <- mrt_sub_Youbike_all %>%
  mutate(
    start_station_clean = clean_station_name(start_station),
    end_station_clean = clean_station_name(end_station)
  )
...
mrt_sub_Youbike_all <- mrt_sub_Youbike_all %>%
  left_join(
    ...
    by = c("start_station_clean" = "sna_clean")
  ) %>%
  left_join(
    bike_coord %>%
      rename(
    ...
    by = c("end_station_clean" = "sna_clean")
  )
...
humidity_panel <- readRDS("Output/RelativeHumidity.rds")
...
#borrow
borrow_data <- Youbike_all %>%
  mutate(
    near_mrt = start_station %in% mrt_bike_station_map$sna,
    hour = hour(start_time)
  ) %>%
  filter(
    near_mrt,
...
return_data <- Youbike_all %>%
  mutate(
    near_mrt = end_station %in% mrt_bike_station_map$sna,
    hour = hour(start_time)
  ) %>%
  filter(
    near_mrt,
...
borrow_hourly <- borrow_data %>%
  count(hour) %>%
  rename(count = n) %>%
  mutate(type = "Borrow")
return_hourly <- return_data %>%
  count(hour) %>%
  rename(count = n) %>%
  mutate(type = "Return")
borrow_return_hourly <- bind_rows(
  borrow_hourly,
  return_hourly
)
...
saveRDS(borrow_return_hourly, "Output/borrow_return_hourly.rds")
